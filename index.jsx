...
class App extends Component {
	...
	request() {
		$.ajax({
			...
			data: JSON.stringify({
				...
				ui,
				condition: {
					productid: window.window.productid,
	...
	beginSearch() {
		...
		$.ajax({
			...
			url: window.baseUrl,
			data: JSON.stringify({
				action: '2001932',//查询出差费用
				ui,
				condition: {
					productid: window.window.productid,
					companyid: window.companyid,
					countryid: s.state.stateId,
					provinceid: s.state.currentProvinceId,
					cityid: s.state.currentCityId,
					traveltime: s.state.startdate,
					traveltime2: s.state.enddate,
					joblevelid: s.state.currentJobId,
					tripcode: s.state.currentReasonId //'h7Wmv3d'//
				}
			})
		}).done((data) => {
			if (typeof data === 'string') {
